...
import { FiUpload, FiEye, FiDollarSign, FiAward, FiPieChart, FiDatabase, FiMail, FiHome, FiBookOpen, FiUsers, FiLock, FiLayers, FiArrowDown } from 'react-icons/fi';
...
const exampleAccounts = [
  ...
  },
  {
    ...
    subscriptions: [
      ...
      { id: 5, name: 'Anytime Fitness', cost: 17.95, frequency: 'Fortnightly', yearly: 466.70 },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <main className="max-w-5xl mx-auto px-4 py-12">
        {/* How It Works - Detailed Vertical Timeline */}
        <motion.div
        ...
        >
          <div className="text-center mb-16">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Our process is simple, transparent, and designed with your privacy in mind.
            </p>
          </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead>
                        <tr>
                          ...
                          <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Yearly Cost</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white divide-y divide-gray-200">
                        {account.subscriptions.map(sub => (
                          <tr key={sub.id}>
                            ...
                            <td className={`px-4 py-2 text-sm font-medium ${sub.yearly > 500 ? 'text-red-600' : 'text-gray-900'}`}>
        ...
        >
          <h2 className="text-3xl font-bold text-gray-900 mb-6 text-center flex items-center justify-center gap-2">
            <FiMail className="inline-block mb-1" /> Example Email Report
          </h2>
          <div className="bg-white rounded-2xl shadow-md border border-gray-200 p-8 max-w-2xl mx-auto">
            <div className="text-center mb-6">
              <h3 className="text-xl font-bold text-gray-900 mb-2">Your Subscription Snapshot</h3>
              <p className="text-gray-600">Here's what your emailed report looks like:</p>
            </div>
            <div className="bg-blue-50 rounded-lg p-4 mb-4">
              <div className="flex items-center justify-center gap-4">
                <div className="flex flex-col items-center">
                  <span className="text-xs text-gray-500">Total Yearly Spend</span>
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-xs text-gray-500">{formatCurrency(mostExpensive.yearly)}/year</span>
                </div>
              </div>
            </div>
            {exampleAccounts.map(account => (
              <div key={account.title} className="mb-6">
                <div className="font-semibold text-gray-800 mb-1">{account.title}</div>
                <table className="w-full text-sm mb-2">
                  <thead>
                    <tr className="bg-blue-100">
                      <th className="py-1 px-2 text-left">Subscription</th>
                      <th className="py-1 px-2 text-left">Amount</th>
                      <th className="py-1 px-2 text-left">Frequency</th>
                      <th className="py-1 px-2 text-left">Yearly</th>
                    </tr>
                  </thead>
                  <tbody>
                    {account.subscriptions.map(sub => (
                      <tr key={sub.id}>
                        <td className="py-1 px-2">{sub.name}</td>
                        <td className="py-1 px-2">{formatCurrency(sub.cost)}</td>
                        <td className="py-1 px-2">{sub.frequency}</td>
                        <td className={`py-1 px-2 ${sub.yearly > 500 ? 'text-red-600' : ''}`}>{formatCurrency(sub.yearly)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="text-center text-xs text-gray-400 mt-6 border-t pt-4">
              <span>🔍 Insights powered by <span className="font-semibold text-blue-700">Snapshot Finance</span></span>
              <br />
              <span>Take control of your recurring expenses</span>
            </div>
          </div>
        </motion.div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-2xl shadow-md border border-gray-200 p-8 flex flex-col items-center text-center">
              ...
              <ul className="text-s text-gray-500 mt-2 space-y-1 text-left">
                <li><span className="font-semibold text-blue-700">✓</span> Combine multiple banks and cards</li>
                <li><span className="font-semibold text-blue-700">✓</span> Spot duplicate subscriptions across accounts</li>
                <li><span className="font-semibold text-blue-700">✓</span> Get one true total for your household</li>
              </ul>
            </div>
            {/* Privacy & Simplicity */}
            <div className="bg-white rounded-2xl shadow-md border border-gray-200 p-8 flex flex-col items-center text-center">
              ...
              <ul className="text-s text-gray-500 mt-2 space-y-1 text-left">
                <li><span className="font-semibold text-green-700">✓</span> No open banking or third-party access</li>
                <li><span className="font-semibold text-green-700">✓</span> You stay in 100% control of your data</li>
                <li><span className="font-semibold text-green-700">✓</span> No passwords, no connections, no risk</li>
              </ul>
            </div>
          </div>
        </motion.div>
